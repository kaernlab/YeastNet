...
if __name__ == "__main__":
    ## Parameters
    datasets = ['DSDataset','DSDataset2','YITDataset1','YITDataset3']
    dataset = datasets[1]
    w0 = 10 # Absolute scaling
    sigma = 5 # Higher means further pixels have greater weight
    ...
    for idx in tqdm(range(timepoints)):
        ...
        elif dataset == 'DSDataset2':
            np.save(folder_name1 + '/lwm{:03d}'.format(idx+20), weight)
            np.save(folder_name1 + '/lwm{:03d}'.format(idx+40), weight)

        imageio.imwrite(folder_name2 + '/lwm{:03d}.png'.format(idx), weight)